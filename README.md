...
| **M4** Analytics collection | 🚧 | Both routes run end to end on the handset: scroll-and-stitch with measured offsets, phase-locked row segmentation, per-field OCR (plus NCC for the rank badges no OCR can read), append-only facts, and uncertain reads triaged in `agent studio`. Against a real 86-row capture the VS gate **passes at 85/86 (98.84%)** against its 95% bar, up from 63/86: every row matched to a member, one queued for review, nothing dropped silently, and the capture still reconciles. What that pass proves is bounded and the gate now says so in its own output: a row counts as correct within ±1%, which at rank 7 is a window ~183,000 wide, so 85/86 means each row reached the right member carrying roughly the right magnitude — **not** that the numbers are right. One of the passing rows is wrong by a single digit. The 1% bar stays anyway: a tighter one would start failing rows for transcription ambiguity in the hand-read fixture itself. The gain came from matching the whole ranking as a **closed set** — one assignment over all rows and all members at once — rather than 86 independent threshold lookups, plus points bounded by the ranking's own order and decoration-stripping in the matcher. Language packs were the obvious fix for the two decorated names and were measured inert. The roster route has its own gate too, `make gate-roster` — ≥95% member coverage with zero splits and zero orphans — and it now **passes at 73/75 (97.33%)**, up from 47/75, with orphans and splits both at zero and all four conditions green. None of the four fixes was a threshold. Rows were attributed to the rank the frame's *sticky* header names, which is wrong on any frame that scrolls across a group boundary: a second group's own header **card** stands inside the list with its rows beneath it, `SegmentRows` deliberately never emits that card as a band — the component that copes with the layout is the one that hid the bug — and finding it by the same NCC badge match the sticky header already uses put nine members back in their own groups. One group's header count read as vertical strokes ("1/11" as "VN") under every geometry, preprocessing shape, threshold and page-segmentation mode ever measured, capping coverage at 64/75 because the count is what gates member creation; every one of those sweeps varied a **luma** operation, and the count separates cleanly on **colour** — the online half is drawn saturated, the total white with a black outline the mask leaves *outside* it, so the touching digits fall apart into addressable column runs. Per-digit classification alone was not safe enough to ship (moving the mask threshold two levels turned a real 11 into a coherent 14), so the reader requires two independent read paths to agree, which takes the whole threshold sweep to zero wrong totals and degrades to refusals instead. The same colour axis then moved the name field, where 48 committed preprocessing sweeps had all been luma: names are saturated orange on a cream card, and reading each band through three saturation masks alongside luma took creatable members from 61 to 73. And the last one was blank space — `nameXFrac1` is a *search* bound, so a short name leaves 40 pixels of empty card inside the crop and the raw-line retry invents glyphs in it ("B52RNI0 ts" for B52RN10, from a crop that is clean on inspection); trimming the crop to its own ink was worth the last two members and the last orphan. The two members it never reaches are Arabic-Indic and Korean names an English tesseract returns nothing usable for. Read the pass knowing what `make probe-roster PROBE_ARGS=-roster.firstwins` says about the margin: production mints a member from the **first** sighting of a row that clears the confidence floor and then stops re-reading that row, so the best band is not the one that decides — Nichoj scored 100 on the best-band view and was an orphan in the gate, because sighting one read "Nicho" at 0.41 and the eleven after it read "Nichoj". `probe-roster`'s first find was the name crop's left edge sitting inside the per-member status icon; moving it to a gutter read off an ink profile and retrying empty reads at raw line was the fix that made a gate worth building. Four instruments are committed across both routes: `make probe-m4`, `make probe-points`, `make probe-assign` — which self-checks with a shuffled-truth canary and decoy padding — and `make probe-roster`, now scored against its own 75-member hand transcription (`fixtures/m4rostergate/expected.yaml`) rather than the VS fixture's. Measured in full in `docs/superpowers/specs/2026-08-17-m4-closed-set-matching-design.md` (VS) and `docs/superpowers/specs/2026-08-20-m4-roster-gate-design.md` (roster). |
