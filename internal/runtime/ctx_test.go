...
func TestVerifyFrameAcceptsAMatchingImage(t *testing.T) {
	c, _ := newCtx(t, &runtimetest.FakeKill{}, "base")
	if err := c.VerifyFrame(context.Background(), "alliance", runtimetest.Frame("alliance")); err != nil {
		t.Fatalf("VerifyFrame: %v", err)
	}
}

func TestVerifyFrameRejectsAMismatchedImage(t *testing.T) {
	c, _ := newCtx(t, &runtimetest.FakeKill{}, "base")
	err := c.VerifyFrame(context.Background(), "alliance", runtimetest.Frame("mail"))
	if !errors.Is(err, runtime.ErrWrongScreen) {
		t.Fatalf("got %v, want ErrWrongScreen", err)
	}
}

// VerifyFrame operates on an image already in hand, not the device's live
// state, so an unrecognized frame is just a mismatch — never a trigger for
// the panic route's back-presses and restarts.
func TestVerifyFrameNeverInvokesThePanicRouteOnAnUnrecognizedFrame(t *testing.T) {
	c, tr := newCtx(t, &runtimetest.FakeKill{}, "base")
	err := c.VerifyFrame(context.Background(), "alliance", runtimetest.Blank())
	if !errors.Is(err, runtime.ErrWrongScreen) {
		t.Fatalf("got %v, want ErrWrongScreen", err)
	}
	if len(tr.Actions()) != 0 {
		t.Fatalf("VerifyFrame touched the device: %+v", tr.Actions())
	}
}

func TestVerifyFrameChecksKillSwitchFirst(t *testing.T) {
	ks := &runtimetest.FakeKill{}
	ks.Set(runtime.ErrPaused)
	c, tr := newCtx(t, ks, "base")
	if err := c.VerifyFrame(context.Background(), "base", runtimetest.Frame("base")); !errors.Is(err, runtime.ErrPaused) {
		t.Fatalf("VerifyFrame: got %v, want ErrPaused", err)
	}
	if len(tr.Actions()) != 0 {
		t.Fatalf("paused ctx touched the device: %+v", tr.Actions())
	}
